...
const getColorScore = (params) => {
    if (params.metric === 'happy') {
        ...
        const closeToBestBy = Math.min(Math.abs(hsl.h - 360 + bestWarmnessHue), Math.abs(hsl.h + bestWarmnessHue));
...
var sub = function sub(pattern, ...values) {
...
(function (module) {
	...
	function _array_remove(array, item) {
	    ...
	    if (idx === -1) throw new TypeError(sub$1('%r not in list', item))
	    array.splice(idx, 1);
	}
	...
	function _choices_to_array(choices) {
	    if (choices === undefined) {
	        ...
	        throw new Error(sub$1('invalid choices value: %r', choices))
	    }
	}
	...
	function _alias(object, from, to) {
	    try {
	        ...
	        Object.defineProperty(object, from, {
	            value: util.deprecate(object[to], sub$1('%s.%s() is renamed to %s.%s()',
	                name, from, name, to)),
	            enumerable: false
	        });
	    } catch {}
	...
	function _parse_opts(args, descriptor) {
	    ...
	    if (renames.length) {
	        let name = get_name();
	        deprecate('camelcase_' + name, sub$1('%s(): following options are renamed: %s',
	            name, renames.map(([ a, b ]) => sub$1('%r -> %r', a, b))));
	    }
	    // end
	    ...
	    for (let [ key, def ] of Object.entries(descriptor)) {
	        if (key[0] === '*') {
	            ...
	            throw new TypeError(sub$1('%s() got multiple values for argument %r', get_name(), key))
	    ...
	    if (Object.keys(kwargs).length) {
	        throw new TypeError(sub$1('%s() got an unexpected keyword argument %r',
	    ...
	    if (args.length) {
	        ...
	        throw new TypeError(sub$1('%s() takes %s positional argument%s but %s %s given',
	            get_name(),
	            from === to ? sub$1('from %s to %s', from, to) : to,
	            from === to && to === 1 ? '' : 's',
	            positional_count,
	    ...
	    if (missing_positionals.length) {
	        ...
	        throw new TypeError(sub$1('%s() missing %i required positional argument%s: %s',
	        ...
	        [util.inspect.custom]() {
	            ...
	            for (let [ name, value ] of this._get_kwargs()) {
	                if (/^[a-z_][a-z0-9_$]*$/i.test(name)) {
	                    arg_strings.push(sub$1('%s=%r', name, value));
	                } else {
	                    star_args[name] = value;
	                }
	            }
	            if (Object.keys(star_args).length) {
	                arg_strings.push(sub$1('**%s', repr(star_args)));
	            }
	            return sub$1('%s(%s)', type_name, arg_strings.join(', '))
	        }
	...
	const HelpFormatter = _camelcase_alias(_callable(class HelpFormatter {
	    ...
	    _format_usage(usage, actions, groups, prefix) {
	        ...
	        if (usage !== undefined) {
	            usage = sub$1(usage, { prog: this._prog });

	        // if no optionals or positionals are available, usage is just prog
	        } else if (usage === undefined && !actions.length) {
	            usage = sub$1('%(prog)s', { prog: this._prog });

	        // if optionals and positionals are available, calculate usage
	        } else if (usage === undefined) {
	            let prog = sub$1('%(prog)s', { prog: this._prog });
	        ...
	        for (let [ i, action ] of Object.entries(actions)) {
	            ...
	            // produce the first way to invoke the option in brackets
	                ...
	                //    -s ARGS or --long ARGS
	                    ...
	                    part = sub$1('%s %s', option_string, args_string);
	                }
	    ...
	    _format_text(text) {
	        if (text.includes('%(prog)')) {
	            text = sub$1(text, { prog: this._prog });
	        }
	    ...
	    _format_action(action) {
	        ...
	        if (!action.help) {
	            ...
	            action_header = sub$1('%*s%s\n', ...tup);

	        // short action name; start on the same line and pad two spaces
	        } else if (action_header.length <= action_width) {
	            let tup = [ this._current_indent, '', action_width, action_header ];
	            action_header = sub$1('%*s%-*s  ', ...tup);
	            indent_first = 0;
	        ...
	        if (!action.option_strings.length) {
	            ...
	            //    -s ARGS, --long ARGS
	                ...
	                for (let option_string of action.option_strings) {
	                    parts.push(sub$1('%s %s', option_string, args_string));
	                }
	            }
	    ...
	    _metavar_formatter(action, default_metavar) {
	        ...
	        if (action.metavar !== undefined) {
	            ...
	            result = sub$1('{%s}', choice_strs.join(','));
	        } else {
	    ...
	    _format_args(action, default_metavar) {
	        ...
	        if (action.nargs === undefined) {
	            result = sub$1('%s', ...get_metavar(1));
	        } else if (action.nargs === OPTIONAL) {
	            result = sub$1('[%s]', ...get_metavar(1));
	        } else if (action.nargs === ZERO_OR_MORE) {
	            let metavar = get_metavar(1);
	            if (metavar.length === 2) {
	                result = sub$1('[%s [%s ...]]', ...metavar);
	            } else {
	                result = sub$1('[%s ...]', ...metavar);
	            }
	        } else if (action.nargs === ONE_OR_MORE) {
	            result = sub$1('%s [%s ...]', ...get_metavar(2));
	            ...
	            result = sub$1(formats.join(' '), ...get_metavar(action.nargs));
	        }
	        return result
	    }
	    ...
	    _expand_help(action) {
	        ...
	        // end
	        return sub$1(this._get_help_string(action), params)
	    }
	...
	const ArgumentError = _callable(class ArgumentError extends Error {
	    ...
	    str() {
	        ...
	        return sub$1(format, { message: this._message,
	...
	const _StoreAction = _callable(class _StoreAction extends Action {
	    ...
	    constructor() {
	        ...
	        if (const_value !== undefined && nargs !== OPTIONAL) {
	            throw new TypeError(sub$1('nargs must be %r to supply const', OPTIONAL))
	        }
	        super({
	            option_strings,
	            dest,
	...
	const _SubParsersAction = _camelcase_alias(_callable(class _SubParsersAction extends Action {
	    ...
	    add_parser() {
	        ...
	        if (kwargs.prog === undefined) {
	            kwargs.prog = sub$1('%s %s', this._prog_prefix, name);
	        }
	    ...
	    call(parser, namespace, values/*, option_string = undefined*/) {
	        ...
	        if (hasattr(this._name_parser_map, parser_name)) {
	            ...
	            let msg = sub$1('unknown parser %(parser_name)r (choices: %(choices)s)', args);
	    ...
	    call(string) {
	        ...
	        if (string === '-') {
	            if (this._flags.includes('r')) {
	                ...
	                let msg = sub$1('argument "-" with mode %r', this._flags);
	        ...
	        let fd;
	        try {
	            fd = fs$1.openSync(string, this._flags, this._options.mode);
	        } catch (e) {
	            let args = { filename: string, error: e.message };
	            let message = "can't open '%(filename)s': %(error)s";
	            throw new ArgumentTypeError(sub$1(message, args))
	        }

	        let options = Object.assign({ fd, flags: this._flags }, this._options);
	        if (this._flags.includes('r')) {
	            return fs$1.createReadStream(undefined, options)
	        } else if (this._flags.includes('w')) {
	            return fs$1.createWriteStream(undefined, options)
	        } else {
	            let msg = sub$1('argument "%s" with mode %r', string, this._flags);
	            throw new TypeError(msg)
	        }
	    }
	    ...
	    [util.inspect.custom]() {
	        ...
	        let args_str = []
	                ...
	                .concat(kwargs.filter(([/*kw*/, arg]) => arg !== undefined)
	                    .map(([kw, arg]) => sub$1('%s=%r', kw, arg)))
	                .join(', ');
	        return sub$1('%s(%s)', this.constructor.name, args_str)
	    }
	...
	const _ActionsContainer = _camelcase_alias(_callable(class _ActionsContainer {
	    ...
	    constructor() {
	        ...
	        ;[ 'storeConst', 'storeTrue', 'storeFalse', 'appendConst' ].forEach(old_name => {
	            ...
	            this.register('action', old_name, util.deprecate(this._registry_get('action', new_name),
	                sub$1('{action: "%s"} is renamed to {action: "%s"}', old_name, new_name)));
	        });
	        // end
	    ...
	    add_argument() {
	        ...
	        if (args.length === 1 && Array.isArray(args[0])) {
	            ...
	            deprecate('argument-array',
	                sub$1('use add_argument(%(args)s, {...}) instead of add_argument([ %(args)s ], { ... })', {
	        ...
	        if (typeof action_class !== 'function') {
	            throw new TypeError(sub$1('unknown action "%s"', action_class))
	        }
	        ...
	        if (typeof type_func !== 'function') {
	            throw new TypeError(sub$1('%r is not callable', type_func))
	        }

	        if (type_func === FileType) {
	            throw new TypeError(sub$1('%r is a FileType class object, instance of it' +
	    ...
	    _add_container_actions(container) {
	        ...
	        for (let group of this._action_groups) {
	            if (group.title in title_group_map) {
	                ...
	                throw new TypeError(sub$1(msg, group.title))
	            }
	    ...
	    _get_optional_kwargs() {
	        ...
	        for (option_string of args) {
	            ...
	            if (!this.prefix_chars.includes(option_string[0])) {
	                ...
	                throw new TypeError(sub$1(msg, args))
	            }
	        ...
	        if (dest === undefined) {
	            ...
	            if (!dest) {
	                ...
	                throw new TypeError(sub$1(msg, option_string))
	            }
	    ...
	    _get_handler() {
	        ...
	        let handler_func_name = sub$1('_handle_conflict_%s', this.conflict_handler);
	        if (typeof this[handler_func_name] === 'function') {
	            return this[handler_func_name]
	        } else {
	            let msg = 'invalid conflict_resolution value: %r';
	            throw new TypeError(sub$1(msg, this.conflict_handler))
	        }
	    }
	    ...
	    _handle_conflict_error(action, conflicting_actions) {
	        ...
	        throw new ArgumentError(action, sub$1(message, conflict_string))
	    }
	...
	const ArgumentParser = _camelcase_alias(_callable(class ArgumentParser extends _AttributeHolder(_ActionsContainer) {
	    ...
	    constructor() {
	        ...
	        if (prog === undefined) {
	            prog = path$1.basename(get_argv()[0] || '');
	        }
	        ...
	        this.register('type', 'int', function (x) {
	            ...
	            if (!Number.isInteger(result)) {
	                throw new TypeError(sub$1('could not convert string to int: %r', x))
	            }
	            return result
	        });
	        this.register('type', 'float', function (x) {
	            let result = Number(x);
	            if (isNaN(result)) {
	                throw new TypeError(sub$1('could not convert string to float: %r', x))
	            }
	            return result
	        });
	    ...
	    parse_args(args = undefined, namespace = undefined) {
	        ...
	        if (argv && argv.length > 0) {
	            ...
	            this.error(sub$1(msg, argv.join(' ')));
	        }
	        return args
	    }
	    ...
	    _parse_known_args(arg_strings, namespace) {
	            ...
	            if (argument_values !== action.default) {
	                ...
	                for (let conflict_action of action_conflicts.get(action) || []) {
	                    if (seen_non_default_actions.has(conflict_action)) {
	                        ...
	                        throw new ArgumentError(action, sub$1(msg, action_name))
	                    }
	                }
	            }
	                ...
	                if (explicit_arg !== undefined) {
	                    ...
	                    if (arg_count === 0 && !chars.includes(option_string[1])) {
	                        ...
	                        if (hasattr(optionals_map, option_string)) {
	                            ...
	                            throw new ArgumentError(action, sub$1(msg, explicit_arg))
	                        }
	        ...
	        if (required_actions.length) {
	            this.error(sub$1('the following arguments are required: %s',
	        ...
	        for (let group of this._mutually_exclusive_groups) {
	            if (group.required) {
	                ...
	                if (no_actions_used) {
	                    ...
	                    this.error(sub$1(msg, names.join(' ')));
	                }
	            }
	        }
	    ...
	    _read_args_from_files(arg_strings) {
	        ...
	        for (let arg_string of arg_strings) {
	            ...
	            // replace arguments referencing files with the file content
	                ...
	                try {
	                    let args_file = fs$1.readFileSync(arg_string.slice(1), 'utf8');
	                    let arg_strings = [];
	    ...
	    _match_argument(action, arg_strings_pattern) {
	        ...
	        if (match === null) {
	            ...
	            if (msg === undefined) {
	                msg = sub$1(action.nargs === 1 ? 'expected %s argument' : 'expected %s arguments', action.nargs);
	            }
	    ...
	    _get_option_tuples(option_string) {
	        ...
	        // shouldn't ever get here
	            ...
	            this.error(sub$1('unexpected option string: %s', option_string));
	        }
	    ...
	    _get_nargs_pattern(action) {
	        ...
	        // all others should be integers
	            ...
	            nargs_pattern = sub$1('(-*%s-*)', 'A'.repeat(nargs).split('').join('-*'));
	        }
	    ...
	    parse_known_intermixed_args(args = undefined, namespace = undefined) {
	        ...
	        if (a.length) {
	            throw new TypeError(sub$1('parse_intermixed_args: positional arg' +
	            ...
	            try {
	                ...
	                for (let action of positionals) {
	                    ...
	                    if (Array.isArray(attr) && attr.length === 0) {
	                        ...
	                        console.warn(sub$1('Do not expect %s in %s', action.dest, namespace));
	    ...
	    _get_value(action, arg_string) {
	        ...
	        if (typeof type_func !== 'function') {
	            ...
	            throw new ArgumentError(action, sub$1(msg, type_func))
	        }
	        ...
	        try {
	            ...
	            // TypeErrors or ValueErrors also indicate errors
	                ...
	                throw new ArgumentError(action, sub$1(msg, args))
	            } else {
	                throw err
	            }
	        }
	    ...
	    error(message) {
	        ...
	        this.exit(2, sub$1('%(prog)s: error: %(message)s\n', args));
	    }
	}));
	    ...
	    get() {
	        ...
	        Object.entries({ ONE_OR_MORE, OPTIONAL, PARSER, REMAINDER, SUPPRESS, ZERO_OR_MORE }).forEach(([ n, v ]) => {
	            Object.defineProperty(result, n, {
	                get() {
	                    deprecate(n, sub$1('use argparse.%s instead of argparse.Const.%s', n, n));
	                    return v
	                }
	            });
	        });
	        Object.entries({ _UNRECOGNIZED_ARGS_ATTR }).forEach(([ n, v ]) => {
	            Object.defineProperty(result, n, {
	                get() {
	                    deprecate(n, sub$1('argparse.Const.%s is an internal symbol and will no longer be available', n));
	                    return v
	                }
	            });
	        });
...
var argparseExports = argparse$1.exports;
